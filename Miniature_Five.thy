...
locale linear_code = code +
  ...
  assumes
    ...
    C_axioms: "\<And> u v. u \<in> C \<Longrightarrow> v \<in> C \<Longrightarrow> elementwise_field_vs.addition F n u v \<in> C"
    "\<And> a v. a \<in> A \<Longrightarrow> v \<in> C \<Longrightarrow> elementwise_field_vs.scaling F n a v \<in> C"
    "elementwise_field_vs.zero_vec F n \<in> C"
begin
...
lemma w_vs: "vectorspace F W" using elementwise_field_vs.vectorspace_VS elem_vs W by metis

lemma code_subspace: "subspace F C W" unfolding subspace_def using
    w_vs
    elementwise_field_vs.vectorspace_VS[OF elem_vs, of n]
    vectorspace_def[of F W]
    words_subs words
    F_carrier
    C_axioms
    elem_vs
  unfolding submodule_def W elementwise_field_vs.VS[OF elem_vs]
  by auto
...
proof -
  ...
  moreover have "carrier W = words"
    unfolding W elementwise_field_vs.VS[OF elem_vs, of n]
...
proof -
  ...
  have "?hammings = { hamming_distance \<zero>\<^bsub>W\<^esub> w | w . w \<in> C \<and> w \<noteq> \<zero>\<^bsub>W\<^esub>}" proof
    ...
    \<subseteq> {hamming_distance \<zero>\<^bsub>W\<^esub> w |w. w \<in> C \<and> w \<noteq> \<zero>\<^bsub>W\<^esub>}" proof
      ...
      have zero_equiv: "\<zero>\<^bsub>CS\<^esub> = \<zero>\<^bsub>W\<^esub>" unfolding W elementwise_field_vs.VS[OF elem_vs, of n] by simp
      ...
      have "\<And>u. u \<in> C \<Longrightarrow> \<ominus>\<^bsub>CS\<^esub> u = \<ominus>\<^bsub>W\<^esub> u" proof -
        ...
        then have "v = (\<ominus>\<^bsub>F\<^esub> \<one>\<^bsub>F\<^esub>) \<odot>\<^bsub>W\<^esub> (vec n (\<lambda>i. u$i))"
          unfolding W elementwise_field_vs.VS[OF elem_vs, of n]
          using elementwise_field_vs.scaling_def[OF elem_vs, of n]
          by auto
        ...
        have "u \<oplus>\<^bsub>CS\<^esub> v = vec n (\<lambda>i. u$i \<oplus>\<^bsub>F\<^esub> v$i)"
          unfolding W elementwise_field_vs.VS[OF elem_vs, of n]
          using elementwise_field_vs.addition_def[OF elem_vs] by simp
        ...
        also have "\<dots> = \<zero>\<^bsub>CS\<^esub>"
          unfolding W elementwise_field_vs.VS[OF elem_vs, of n]
          using elementwise_field_vs.zero_vec_def[OF elem_vs]
          by simp
      ...
      moreover have "(?u \<ominus>\<^bsub>CS\<^esub> ?v) \<noteq> \<zero>\<^bsub>CS\<^esub>" proof (rule ccontr)
        ...
        then have "inv\<^bsub>add_monoid CS\<^esub> (\<ominus>\<^bsub>CS\<^esub> ?v) = ?u"
          ...
          unfolding elementwise_field_vs.VS[OF elem_vs, of n] W
          by auto
  ...
  next
    ...
    \<subseteq> {hamming_distance (fst p) (snd p) |p. p \<in> C \<times> C \<and> fst p \<noteq> snd p}" proof
      ...
      then have "(\<zero>\<^bsub>W\<^esub>, w) \<in> C \<times> C"
        unfolding W elementwise_field_vs.VS
        using elem_vs C_axioms elementwise_field_vs.VS
        by (simp add: elementwise_field_vs.VS)
